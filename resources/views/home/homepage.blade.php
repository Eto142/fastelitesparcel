...
<div class="ot-ticker-outer">
    <style>
        .ot-ticker-outer{background:#0a2010;overflow:hidden;position:relative;z-index:100;border-bottom:1px solid rgba(26,90,46,.3);}
        .ot-ticker-inner-wrap{display:flex;align-items:center;height:40px;}
        .ot-ticker-badge{background:#f4a306;color:#0d2e17;font-size:10px;font-weight:800;letter-spacing:2px;text-transform:uppercase;padding:0 18px;height:100%;display:flex;align-items:center;white-space:nowrap;flex-shrink:0;gap:6px;}
...
<!-- ===== STATS COUNTER STRIP ===== -->
<section class="ot-stats-section">
    <style>
        .ot-stats-section{
            background:linear-gradient(135deg,#0a2010 0%,#0d2e17 50%,#112a1a 100%);
            padding:64px 20px;position:relative;overflow:hidden;
        }
        .ot-stats-section::before{
            content:'';position:absolute;inset:0;
            background:radial-gradient(ellipse 80% 60% at 50% 50%,rgba(26,90,46,.25) 0%,transparent 70%);
            pointer-events:none;
        }
        .ot-stats-wrap{max-width:1100px;margin:0 auto;display:grid;grid-template-columns:repeat(4,1fr);gap:20px;position:relative;}
        @media(max-width:900px){.ot-stats-wrap{grid-template-columns:repeat(2,1fr);}}
        @media(max-width:480px){.ot-stats-wrap{grid-template-columns:1fr 1fr;gap:14px;}}
        .ot-stat-card{
            background:rgba(255,255,255,.04);
            border:1px solid rgba(255,255,255,.08);
            border-radius:12px;padding:32px 20px 28px;
            text-align:center;position:relative;overflow:hidden;
            transition:transform .3s ease,box-shadow .3s ease;
        }
        .ot-stat-card:hover{transform:translateY(-4px);box-shadow:0 12px 40px rgba(0,0,0,.4);}
        .ot-stat-card::before{
            content:'';position:absolute;top:0;left:10%;right:10%;height:2px;
            border-radius:0 0 2px 2px;
            background:linear-gradient(90deg,transparent,#f4a306,transparent);
        }
        .ot-stat-card-glow{
            position:absolute;bottom:-20px;left:50%;transform:translateX(-50%);
            width:80px;height:80px;border-radius:50%;
            background:radial-gradient(circle,rgba(244,163,6,.12) 0%,transparent 70%);
        }
        .ot-stat-icon{font-size:22px;color:rgba(244,163,6,.8);margin-bottom:14px;}
        .ot-stat-num{
            font-size:clamp(38px,5vw,52px);font-weight:900;
            color:#f4a306;line-height:1;
            font-family:'Rajdhani','Arial',sans-serif;
            text-shadow:0 0 20px rgba(244,163,6,.3);
        }
        .ot-stat-label{
            font-size:11px;color:rgba(255,255,255,.6);
            letter-spacing:2px;text-transform:uppercase;
            margin-top:10px;font-weight:600;line-height:1.5;
        }
    </style>
    <div class="ot-stats-wrap">
        <div class="ot-stat-card">
            <div class="ot-stat-card-glow"></div>
            <div class="ot-stat-icon"><i class="fas fa-box"></i></div>
            <div class="ot-stat-num"><span class="ot-counter" data-to="50" data-suffix="K+">0</span></div>
            <div class="ot-stat-label">Shipments Delivered</div>
        </div>
        <div class="ot-stat-card">
            <div class="ot-stat-card-glow"></div>
            <div class="ot-stat-icon"><i class="fas fa-globe-americas"></i></div>
            <div class="ot-stat-num"><span class="ot-counter" data-to="80" data-suffix="+">0</span></div>
            <div class="ot-stat-label">Countries Served</div>
        </div>
        <div class="ot-stat-card">
            <div class="ot-stat-card-glow"></div>
            <div class="ot-stat-icon"><i class="fas fa-award"></i></div>
            <div class="ot-stat-num"><span class="ot-counter" data-to="10" data-suffix="+">0</span></div>
            <div class="ot-stat-label">Years of Experience</div>
        </div>
        <div class="ot-stat-card">
            <div class="ot-stat-card-glow"></div>
            <div class="ot-stat-icon"><i class="fas fa-check-circle"></i></div>
            <div class="ot-stat-num"><span class="ot-counter" data-to="98" data-suffix="%">0</span></div>
            <div class="ot-stat-label">On-Time Delivery Rate</div>
        </div>
    </div>
    <script>
    (function(){
        var counters = document.querySelectorAll('.ot-counter');
        var triggered = false;
        function runCounters(){
            ...
            counters.forEach(function(el){
                ...
                function step(ts){
                    ...
                    var progress = Math.min((ts - startTime) / 1800, 1);
        ...
        if('IntersectionObserver' in window){
            ...
            var sec = document.querySelector('.ot-stats-section');
            if(sec) obs.observe(sec);
        } else { window.addEventListener('scroll', runCounters, {passive:true}); }
    })();
    </script>
</section>
...
<section class="ot-hiw-section">
    <style>
        ...
        .ot-hiw-header h2{font-size:clamp(26px,3.5vw,36px);font-weight:800;color:#0d2e17;margin-bottom:14px;line-height:1.2;}
        ...
        .ot-step-line{position:absolute;top:35px;left:12.5%;right:12.5%;height:0;border-top:2px dashed rgba(26,90,46,.15);z-index:0;}
        .ot-hiw-step{text-align:center;padding:0 16px;position:relative;z-index:1;}
        .ot-hiw-num{
            width:72px;height:72px;border-radius:50%;
            background:linear-gradient(135deg,#e8f5ec,#d4edde);
            border:2px solid rgba(26,90,46,.15);
            display:flex;align-items:center;justify-content:center;
            margin:0 auto 20px;position:relative;
            transition:transform .3s,box-shadow .3s;
        }
        .ot-hiw-step:hover .ot-hiw-num{transform:scale(1.08);box-shadow:0 8px 24px rgba(26,90,46,.2);}
        .ot-hiw-num i{font-size:26px;color:#1a5a2e;}
        .ot-hiw-step-num{
        ...
        .ot-hiw-title{font-size:16px;font-weight:700;color:#0d2e17;margin-bottom:10px;}
        .ot-hiw-desc{font-size:13.5px;color:rgba(20,40,20,.6);line-height:1.7;max-width:220px;margin:0 auto;}
    </style>
...
<div class="elementor-widget-container">
<div class="elementor-heading-title elementor-size-default">fastelitesparcel prides itself on its customer-centric focus, safety supremacy, and partnership formation. We are big enough to deliver extensive global logistics services, yet agile enough to provide the personal attention every client deserves.</div></div></div></div></div></div></section>
...
<div class="elementor-widget-container">
<div class="elementor-heading-title elementor-size-default"><a href="custom-shipping-crates/index.html">fastelitesparcel crates and ships heavy, oversized, and fragile items, including fine china and crystal, paintings and sculptures, machinery, and industrial equipment.</a></div></div></div><div class="elementor-element elementor-element-4ac7de6 elementor-align-left elementor-widget elementor-widget-button" data-id="4ac7de6" data-element_type="widget" data-widget_type="button.default">
<div class="elementor-widget-container">
...
<div class="elementor-widget-container">
<div class="elementor-heading-title elementor-size-default"><a href="specialty-freight/index.html">fastelitesparcel offers innovative solutions for specialty freight challenges. We pack, crate, and ship, art, machinery, antiques, furniture, and equipment all over the world. Our specialists will provide consultation on the most efficient, time-sensitive, and cost-effective manner to pack and ship your specialty freight worldwide.</a></div></div></div>
...
<section class="elementor-section elementor-top-section elementor-element elementor-section-boxed elementor-section-height-default" style="background:#f4f6f9;padding:70px 0;">
    <style>
        ...
        .ot-testimonials-header h2{font-size:32px;font-weight:700;color:#0d2e17;margin-bottom:14px;}
        ...
        .ot-testimonial-avatar{width:48px;height:48px;border-radius:50%;background:#0d2e17;color:#fff;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;flex-shrink:0;}
        .ot-testimonial-name{font-weight:700;color:#0d2e17;font-size:15px;}
    ...
    <div class="ot-testimonials-wrap">
        ...
        <div class="ot-testimonials-grid">
            <div class="ot-testimonial-card">
                ...
                <p class="ot-testimonial-text">fastelitesparcel handled our international shipment flawlessly. Real-time tracking kept us informed every step of the way, and our goods arrived two days ahead of schedule. Absolutely outstanding service.</p>
            ...
            <div class="ot-testimonial-card">
                <div class="ot-testimonial-stars">&#9733;&#9733;&#9733;&#9733;&#9733;</div>
                <p class="ot-testimonial-text">I was nervous about shipping my pet internationally, but the team at fastelitesparcel made the entire process stress-free. They communicated every detail and my dog arrived safe and happy. Highly recommend!</p>
                <div class="ot-testimonial-author">
            ...
            <div class="ot-testimonial-card">
                <div class="ot-testimonial-stars">&#9733;&#9733;&#9733;&#9733;&#9733;</div>
                <p class="ot-testimonial-text">We have been partnering with fastelitesparcel for over three years for all our freight needs. Their dedication to meeting deadlines and their professional team have made them an invaluable part of our supply chain.</p>
                <div class="ot-testimonial-author">
            ...
            <div class="ot-testimonial-card">
                <div class="ot-testimonial-stars">&#9733;&#9733;&#9733;&#9733;&#9733;</div>
                <p class="ot-testimonial-text">Moving to Norway was daunting, but fastelitesparcel handled our household belongings with exceptional care. Every item arrived in perfect condition. The team was responsive and professional throughout.</p>
                <div class="ot-testimonial-author">
            ...
            <div class="ot-testimonial-card">
                <div class="ot-testimonial-stars">&#9733;&#9733;&#9733;&#9733;&#9733;</div>
                <p class="ot-testimonial-text">From custom shipping crates to final-mile delivery, fastelitesparcel covered everything for our exhibition equipment. They truly deliver confidence exactly as they promise. A partner we trust completely.</p>
                <div class="ot-testimonial-author">
...
<section style="background:linear-gradient(90deg,#f4a306 0%,#e8940a 100%);padding:60px 20px;text-align:center;">
    <style>
        ...
        .ot-cta-wrap h2{font-size:clamp(22px,4vw,36px);font-weight:800;color:#0d2e17;margin:0 0 14px;line-height:1.25;}
        .ot-cta-wrap p{font-size:16px;color:rgba(13,46,23,0.8);margin:0 0 28px;line-height:1.6;}
        .ot-cta-btn{display:inline-block;background:#0d2e17;color:#f4a306;font-size:14px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;padding:15px 36px;border-radius:4px;text-decoration:none;transition:background 0.3s,color 0.3s;}
        .ot-cta-btn:hover{background:#fff;color:#0d2e17;}
    </style>
    <div class="ot-cta-wrap">
        ...
        &nbsp;&nbsp;
        <a class="ot-cta-btn" href="{{ url('/track-now') }}" style="background:transparent;border:2px solid #0d2e17;color:#0d2e17;"><i class="fas fa-map-marker-alt" style="margin-right:8px;"></i>Track Shipment</a>
    </div>
</section>
